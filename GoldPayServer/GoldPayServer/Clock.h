...
{
	std::time_t		serverStartTick_;

	std::wstring	tickToStr(std::time_t	 tick, const WCHAR *fmt = DATETIME_FORMAT);

public:
	Clock();
	~Clock();

	std::time_t		serverStartTick();
	std::time_t		systemTick();
	std::time_t		strToTick(std::wstring str, const WCHAR *fmt = DB_TIME_FORMAT);

	std::wstring	nowTime(const WCHAR *fmt = DATETIME_FORMAT);
	std::wstring	nowTimeWithMilliSec(const WCHAR *fmt = DATETIME_FORMAT);

	std::wstring today();
	std::wstring tomorrow();
	std::wstring yesterday();
